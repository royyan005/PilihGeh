<?php
    ...
    if(isset($_POST['submit'])){
        if(is_numeric($_POST['angkatan'])){
            if(is_numeric($_POST['username'])){
               if(empty(DB::query('SELECT username FROM datavoter WHERE username=:username', array(':username'=>$_POST['username'])))){
                $nama = $_POST['name'];
                $username = $_POST['username'];
                $angkatan = $_POST['angkatan'];
                $token = 1;
                    
                DB::query('INSERT INTO datavoter VALUES (\'\', :nama, :username, :password, :angkatan, :token)', array(':nama'=>$nama, ':username'=>$username, ':password'=>$username, ':angkatan'=>$angkatan, ':token'=>$token));
                $pemberitahuan = "Data Berhasil Ditambahkan!";
                } else {
                    $pemberitahuan = "Username Telah Terdaftar!";
                }
            } else {
               $pemberitahuan = "Username Harus Berupa Angka(NPM)!";
            }
        } else {
                        ...
                        <div class="card-body">
                            <form action="" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" placeholder="Angkatan" name="angkatan" required>
                                </div>
